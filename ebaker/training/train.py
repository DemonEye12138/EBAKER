...
def train_one_epoch(
    ...
    top_10_percent_index = int(args.dropratio * model.queuesize)
    sortedsim,_ = torch.sort(model.queuesim)
    model.threshold = sortedsim[top_10_percent_index]
    for i, batch in enumerate(dataloader):
        ...
        with autocast():
            with forward_context():
                ...
                image_features, text_features, logit_scale,text_tokens,image_tokens,local2,mlm_scores,mlm_labels,threshold  = model(images, texts, text_only=(args.cache_teacher is not None) or args.w_align==0)#
                ...
                if args.w_simcse > 0:
                    ...
                    ssl_loss = args.w_simcse * simcse_contrastive_loss(image_features, text_features,local2,mlm_scores,mlm_labels,epoch,threshold ,logit_scale=np.exp(2.996))
                else:
                    ssl_loss = 0
